      subroutine invtau_nk_Z (nk1,nk2,nk3,nbnd,nksfit,nmod,etfit,       &
                  xk,vk,dfk,ind_k,ind_ph,eqk,g2,wo,ef,T,at,bg,          &
                  aa, tauk )
      !
...
!$    USE omp_lib
     ...
     &                       ind_k,ind_ph
      ...
      double precision, intent(in) :: etfit(nbnd,nksfit),               &
     &                                xk(3), ef, T, aa, at(3,3),bg(3,3),&
     &                                vk(nbnd,nk1*nk2*nk3,3),           &
     &                                dfk(nbnd,nk1*nk2*nk3,3),          &
     &                                g2(nksfit,nmod), wo(nksfit,nmod)
      ...
      double precision :: wq, dfkq(nbnd,nk1*nk2*nk3,3),                 &
     ...
     &                    norm_vkq,vfac,vkk(3),dfkk(3)
      ...
      !$omp reduction(+:tauk)
      do iq=2,nkfit ! No q2=0 divergence, but zero freq phonons at gamma
         !
         do jbnd=1,nbnd
            !
            ...
            ! 
            ! Sum over Optical modes
            do nu=4,nmod
               !
               ! BE distribution
               be = 1.d0 / ( exp(wo(eqk(iq),nu)/T) - 1.d0 + 1e-10)   
               !
               ! n_{q\nu} + f_{j,k+q}
               ...
               if ( deg .le. 1d-10 ) then     
                  ...
                  w0g1 = w0gauss((ekq-ek-wo(eqk(iq),nu))/deg)/deg
                  w0g2 = w0gauss((ekq-ek+wo(eqk(iq),nu))/deg)/deg
               end if
               !
               ! Velocity factor
               ...
               ! Compute tauk 
               !vfac = 1.0 ! DEBUG TEST
               tauk = tauk + tpi * wq * g2(eqk(iq),nu) * vfac * ( temp1 * w0g1 + temp2 * w0g2 )
               !
            end do ! nu 
            ! 
         end do ! jbnd
